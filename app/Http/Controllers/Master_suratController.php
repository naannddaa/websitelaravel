   ...
   public function index(Request $request)
{
    ...
    public function store(Request $request)
    {
        ...
        $request->image->move(public_path('storage/surat'), $imageName);
    ...
    public function update(Request $request, $id_surat)
{
   ...
   if ($request->hasFile('image')) {
        if ($surat->image && file_exists(public_path('storage/surat/' . $surat->image))) {
            unlink(public_path('storage/surat/' . $surat->image));
        }

        $imageName = time() . '.' . $request->image->extension();
        $request->image->storeAs('surat', $imageName, 'public'); // Menyimpan dengan Storage facade

       $data['image'] = $imageName;
    } else {
        $data['image'] = $surat->image;
    }
